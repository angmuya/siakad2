class FileUploader{
  constructor(el, currentFile, filename, accept){
    this.currentFile = currentFile;
    this.el = el;
    this.filename = filename;
    this.accept = accept;
    this.defaultFileName = 'Tidak ada';
    this.render();
    this.init();
    this.assignControl();
  }  
  ...
  render(){
    this.el.html(`
      <div class="form-group">
        <label for="${this.filename}Filename">Dokumen ${capFirstLetter(this.filename)}</label>
        <div class="input-group">
          <input type="text" class="form-control" id="${this.filename}Filename" name="${this.filename}Filename" readonly>
          <div class="input-group-append">
            <button class="btn btn-outline-success" type="button" id="downloadFileButton" style="display:none" data-toggle="tooltip" data-placement="top" title="Unduh"><i class="fa fa-download"></i></button>
            <button class="btn btn-outline-primary" type="button" id="browseFileButton" data-toggle="tooltip" data-placement="top" title="Pilih ${capFirstLetter(this.filename)} File"><i class="fa fa-folder-open"></i></button>
            <button class="btn btn-outline-danger" type="button" id="deleteFileButton" style="display:none" data-toggle="tooltip" data-placement="top" title="Hapus ${capFirstLetter(this.filename)} File"><i class="fa fa-trash"></i></button>
            <button class="btn btn-outline-warning" type="button" id="cancelFileButton" style="display:none" data-toggle="tooltip" data-placement="top" title="Batal"><i class="fa fa-ban"></i></button>
          </div>
        </div>
  ...
  assignControl(){
    ...
    this.downloadFileButton.on('click', function(){
      window.open(`../../uploads/${context.filename}/` + context.fileFilename.val());
    });
